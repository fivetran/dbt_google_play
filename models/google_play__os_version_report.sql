with installs as (
    ...
    select *
    from {{ ref('stg_google_play__stats_installs_os_version') }}
), 

ratings as (

    select *
    from {{ ref('stg_google_play__stats_ratings_os_version') }}
), 

crashes as (

    select *
    from {{ ref('stg_google_play__stats_crashes_os_version') }}
), 
...
install_metrics as (
    ...
    select
        *,
        sum(device_installs) over (partition by android_os_version, package_name {{ fivetran_utils.partition_by_source_relation(package_name='google_play') }} order by date_day asc rows between unbounded preceding and current row) as total_device_installs,
        sum(device_uninstalls) over (partition by android_os_version, package_name {{ fivetran_utils.partition_by_source_relation(package_name='google_play') }} order by date_day asc rows between unbounded preceding and current row) as total_device_uninstalls
    from installs 
), 
...
create_partitions as (
    ...
    {% for metric in rolling_metrics -%}
        , sum(case when {{ metric }} is null 
                then 0 else 1 end) over (partition by android_os_version, package_name {{ fivetran_utils.partition_by_source_relation(package_name='google_play') }} order by date_day asc rows unbounded preceding) as {{ metric | lower }}_partition
    {%- endfor %}
...
fill_values as (
    ...
    select 
        ...
        , first_value( {{ metric }} ) over (
            partition by {{ metric | lower }}_partition, android_os_version, package_name {{ fivetran_utils.partition_by_source_relation(package_name='google_play') }} order by date_day asc rows between unbounded preceding and current row) as {{ metric }}
