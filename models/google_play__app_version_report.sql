...
app_version_join as (
    ...
    full outer join ratings
        ...
        and coalesce(install_metrics.app_version_code, 'null_app_version') = coalesce(ratings.app_version_code, 'null_app_version') -- this really doesn't happen IRL but let's be safe
    full outer join crashes
        on install_metrics.date_day = crashes.date_day
        and install_metrics.package_name = crashes.package_name
        and coalesce(install_metrics.app_version_codem 'null_app_version') = coalesce(crashes.app_version_code, 'null_app_version')
), 
